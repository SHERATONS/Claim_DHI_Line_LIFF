...
func NewOtherClaimHandler(repo claim.OtherClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *OtherClaimHandler {
	return &OtherClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *OtherClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[OtherClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[OtherClaim] Failed to stream to GCS: %v", err)
				continue
			}
