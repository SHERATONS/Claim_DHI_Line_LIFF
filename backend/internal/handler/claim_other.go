...
type OtherClaimHandler struct {
	repo    claim.OtherClaimRepository
	upload  domain.UploadRepository
	storage domain.StorageRepository
	gen     domain.ContentGenerator
}

func NewOtherClaimHandler(repo claim.OtherClaimRepository, upload domain.UploadRepository, storage domain.StorageRepository, gen domain.ContentGenerator) *OtherClaimHandler {
	return &OtherClaimHandler{repo: repo, upload: upload, storage: storage, gen: gen}
}

type otherClaimForm struct {
...
type otherClaimResponse struct {
	Success bool             `json:"success"`
	Data    *otherResultData `json:"data,omitempty"`
	Error   string           `json:"error,omitempty"`
}

type otherResultData struct {
	NotificationNo string `json:"notificationNo"`
	CaseNumber     string `json:"caseNumber"`
	CaseId         string `json:"caseId"`
}

func (h *OtherClaimHandler) Handle(c *gin.Context) {
	var form otherClaimForm
	if err := c.ShouldBind(&form); err != nil {
		httpresponse.BadRequest(c, fmt.Errorf("invalid request body: %w", err))
		return
	}

	req := claim.OtherClaimRequest{
	...
	if len(fileInputs) > 0 {
		...
		if err != nil {
			log.Printf("[OtherClaim] AI Analysis failed: %v", err)
	...
	if err != nil {
		log.Printf("[OtherClaim] ERROR: %v", err)
	...
	if result.Success && result.CaseId != "" && len(fileInputs) > 0 {
		...
		if len(uploadErrors) > 0 {
			...
			log.Printf("[OtherClaim] Partial success: %s", result.Error)
		}
	}

	c.JSON(http.StatusOK, otherClaimResponse{
		Success: result.Success,
		Data:    &otherResultData{NotificationNo: result.NotificationNo, CaseNumber: result.CaseNumber, CaseId: result.CaseId},
		Error:   result.Error,
	})
}
