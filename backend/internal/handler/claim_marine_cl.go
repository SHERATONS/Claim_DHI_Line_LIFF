...
func NewMarineCLClaimHandler(repo claim.MarineCLClaimRepository, upload domain.UploadRepository, storage domain.StorageRepository, gen domain.ContentGenerator) *MarineCLClaimHandler {
	return &MarineCLClaimHandler{repo: repo, upload: upload, storage: storage, gen: gen}
}
...
func (h *MarineCLClaimHandler) Handle(c *gin.Context) {
	...
	if len(fileInputs) > 0 {
		...
		if err != nil {
			log.Printf("[MarineCLClaim] AI Analysis failed: %v", err)
