...
func NewMarineCLClaimHandler(repo claim.MarineCLClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *MarineCLClaimHandler {
	return &MarineCLClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *MarineCLClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[MarineCLClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[MarineCLClaim] Failed to stream to GCS: %v", err)
				continue
			}
