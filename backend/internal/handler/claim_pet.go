...
func NewPetClaimHandler(repo claim.PetClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *PetClaimHandler {
	return &PetClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *PetClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[PetClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[PetClaim] Failed to stream to GCS: %v", err)
				continue
			}
