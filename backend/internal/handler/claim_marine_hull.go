...
func NewMarineHullClaimHandler(repo claim.MarineHullClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *MarineHullClaimHandler {
	return &MarineHullClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *MarineHullClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[MarineHullClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[MarineHullClaim] Failed to stream to GCS: %v", err)
				continue
			}
