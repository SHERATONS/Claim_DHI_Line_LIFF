...
type MarineHullClaimHandler struct {
	repo    claim.MarineHullClaimRepository
	upload  domain.UploadRepository
	storage domain.StorageRepository
	gen     domain.ContentGenerator
}

func NewMarineHullClaimHandler(repo claim.MarineHullClaimRepository, upload domain.UploadRepository, storage domain.StorageRepository, gen domain.ContentGenerator) *MarineHullClaimHandler {
	return &MarineHullClaimHandler{repo: repo, upload: upload, storage: storage, gen: gen}
}

type marineHullClaimForm struct {
	...
	BoatName         string `form:"boatName"`
	CauseOfLoss      string `form:"causeOfLoss"`
	LossReserve      string `form:"lossReserve"`
}

type marineHullClaimResponse struct {
	Success bool                  `json:"success"`
	Data    *marineHullResultData `json:"data,omitempty"`
	Error   string                `json:"error,omitempty"`
}

type marineHullResultData struct {
	NotificationNo string `json:"notificationNo"`
	CaseNumber     string `json:"caseNumber"`
	CaseId         string `json:"caseId"`
}

func (h *MarineHullClaimHandler) Handle(c *gin.Context) {
	var form marineHullClaimForm
	if err := c.ShouldBind(&form); err != nil {
		httpresponse.BadRequest(c, fmt.Errorf("invalid request body: %w", err))
		return
	}

	req := claim.MarineHullClaimRequest{
	...
	if len(fileInputs) > 0 {
		...
		if err != nil {
			log.Printf("[MarineHullClaim] AI Analysis failed: %v", err)
	...
	if err != nil {
		log.Printf("[MarineHullClaim] ERROR: %v", err)
	...
	if result.Success && result.CaseId != "" && len(fileInputs) > 0 {
		...
		if len(uploadErrors) > 0 {
			...
			log.Printf("[MarineHullClaim] Partial success: %s", result.Error)
		}
	}

	c.JSON(http.StatusOK, marineHullClaimResponse{
		Success: result.Success,
		Data:    &marineHullResultData{NotificationNo: result.NotificationNo, CaseNumber: result.CaseNumber, CaseId: result.CaseId},
		Error:   result.Error,
	})
}
