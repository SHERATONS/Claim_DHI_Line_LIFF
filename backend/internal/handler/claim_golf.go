...
func NewGolfClaimHandler(repo claim.GolfClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *GolfClaimHandler {
	return &GolfClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *GolfClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[GolfClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[GolfClaim] Failed to stream to GCS: %v", err)
				continue
			}
