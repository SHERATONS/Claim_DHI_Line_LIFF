...
type GolfClaimHandler struct {
	repo    claim.GolfClaimRepository
	upload  domain.UploadRepository
	storage domain.StorageRepository
	gen     domain.ContentGenerator
}

func NewGolfClaimHandler(repo claim.GolfClaimRepository, upload domain.UploadRepository, storage domain.StorageRepository, gen domain.ContentGenerator) *GolfClaimHandler {
	return &GolfClaimHandler{repo: repo, upload: upload, storage: storage, gen: gen}
}

type golfClaimForm struct {
...
type golfClaimResponse struct {
	Success bool            `json:"success"`
	Data    *golfResultData `json:"data,omitempty"`
	Error   string          `json:"error,omitempty"`
}

type golfResultData struct {
	NotificationNo string `json:"notificationNo"`
	CaseNumber     string `json:"caseNumber"`
	CaseId         string `json:"caseId"`
}

func (h *GolfClaimHandler) Handle(c *gin.Context) {
	var form golfClaimForm
	if err := c.ShouldBind(&form); err != nil {
		httpresponse.BadRequest(c, fmt.Errorf("invalid request body: %w", err))
		return
	}

	req := claim.GolfClaimRequest{
	...
	if len(fileInputs) > 0 {
		...
		if err != nil {
			log.Printf("[GolfClaim] AI Analysis failed: %v", err)
	...
	if err != nil {
		log.Printf("[GolfClaim] ERROR: %v", err)
	...
	if result.Success && result.CaseId != "" && len(fileInputs) > 0 {
		...
		if len(uploadErrors) > 0 {
			...
			log.Printf("[GolfClaim] Partial success: %s", result.Error)
		}
	}

	c.JSON(http.StatusOK, golfClaimResponse{
		Success: result.Success,
		Data:    &golfResultData{NotificationNo: result.NotificationNo, CaseNumber: result.CaseNumber, CaseId: result.CaseId},
		Error:   result.Error,
	})
}
