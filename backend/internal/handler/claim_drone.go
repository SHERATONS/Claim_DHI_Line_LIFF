...
func NewDroneClaimHandler(repo claim.DroneClaimRepository, worker *service.AsyncWorker, gcsClient *gcs.Client) *DroneClaimHandler {
	return &DroneClaimHandler{repo: repo, worker: worker, gcs: gcsClient}
}
...
func (h *DroneClaimHandler) Handle(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(32 << 20); err == nil {
		...
		for _, fileHeader := range files {
			...
			if err != nil {
				log.Printf("[DroneClaim] Failed to open file: %v", err)
				continue
			}
			defer file.Close()
			...
			if err != nil {
				log.Printf("[DroneClaim] Failed to stream to GCS: %v", err)
				continue
			}
