...
func NewDroneClaimHandler(repo claim.DroneClaimRepository, upload domain.UploadRepository, storage domain.StorageRepository, gen domain.ContentGenerator) *DroneClaimHandler {
	return &DroneClaimHandler{repo: repo, upload: upload, storage: storage, gen: gen}
}
...
func (h *DroneClaimHandler) Handle(c *gin.Context) {
	...
	if len(fileInputs) > 0 {
		...
		if err != nil {
			log.Printf("[DroneClaim] AI Analysis failed: %v", err)
