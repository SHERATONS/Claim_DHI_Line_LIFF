/**
 * Claim Form Page (Marine Cargo)
 * Adapted from Marine_HULL_Claim
 */

import { useState } from 'react';
import { useLiff, useFileUpload } from '@/hooks';
import { marineCargoSchema } from '@/utils/validation';
...
export default function ClaimForm() {
    ...
    const validateForm = (): boolean => {
        const result = marineCargoSchema.safeParse({ ...values });
        if (result.success) {
            setErrors({});
            return true;
        }
    ...
    const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (USE_LOCAL_SAVE) {
                ...
                await saveClaimLocally({ claimType: 'Marine_CARGO_Claim', data: payload, documentFiles: allFiles });
    ...
    if (submitState.success) {
        const contactEmail = getContactEmail('marine_cargo');
        return (
            <SuccessScreen
        ...
        <>
            ...
            <div className="header-section">
                ...
                <div style={{ fontSize: '0.95rem', marginTop: 5 }}>
                    แจ้งเคลม Marine Cargo (สอบถามข้อมูลเพิ่มเติม กรุณาติดต่อ Call Center 1736)
                </div>
            </div>
